...
/**
 * A "Small:180|Medium:250|Large:320" cell into the array shape every
 * variation/add-on already uses everywhere else — the POS editor, the
 * pricing engine, an order line's variationId/addonIds.
 *
 * Without this, a dish with sizes has no way to arrive as ONE row: the only
 * options a flat spreadsheet leaves are three separate rows ("Pizza Small",
 * "Pizza Medium", "Pizza Large"), which import as three unrelated menu
 * items rather than one item a diner picks a size for.
 *
 * A malformed sub-entry (no name, or a price that doesn't clean up to a
 * number) is dropped rather than failing the whole row — one bad size
 * shouldn't cost the dish its other two.
 *
 * A JSON import can give this a real array of {name, price} objects instead
 * of a pipe-delimited string — JSON has no reason to flatten structured data
 * into text just because CSV does — so both shapes are accepted directly.
 */
export function parseVariationsAddons(raw, prefix = "opt") {
  if (!raw) return [];
  if (Array.isArray(raw)) {
    return raw.map((r, i) => {
      ...
      return { id: r?.id || `${prefix}${i}`, name, price };
    }).filter(Boolean);
  }
  return String(raw).split("|")
    ...
    .map((part, i) => {
      ...
      const name = (sep === -1 ? part : part.slice(0, sep)).trim();
      const price = cleanPrice(sep === -1 ? "" : part.slice(sep + 1));
      if (!name || price === null) return null;
      return { id: `${prefix}${i}`, name, price };
    })
    .filter(Boolean);
}
...
export function normalizeMenuRows(rows) {
  ...
  (rows || []).forEach((row, i) => {
    ...
    items.push({
      ...
      // Only meaningful for a zip import — matched against the archive's
      // photos by the caller. Harmless and unused on a plain CSV/JSON import.
      imageFile: pick(row, "imagefile", "image file"),
      ...
      bogoEnabled: truthy(pick(row, "bogo", "bogoenabled", "buy1get1", "buy 1 get 1")),
      // One row, one dish — sizes and add-ons live inside that one row as
      // "Small:180|Medium:250|Large:320" rather than as separate rows, which
      // is what would otherwise turn "Pizza" into three unrelated items.
      // Read directly off the row (not through pick(), which stringifies)
      // so a JSON import's real array of {name, price} objects survives —
      // row keys are already lowercased above, for both CSV and JSON.
      variations: parseVariationsAddons(row.variations || row.sizes, "var"),
      addons: parseVariationsAddons(row.addons || row["add-ons"] || row["add ons"], "add"),
    });
  });
...
export const MENU_CSV_TEMPLATE =
  ...
  "Paneer Tikka,280,Starters,Char-grilled cottage cheese,veg,true,false,,,,15,false\n" +
  "Margherita Pizza,320,Mains,Classic tomato and mozzarella,veg,false,true,,\"Small:220|Medium:320|Large:420\",\"Extra Cheese:40|Extra Olives:30\",20,false\n" +
  "Butter Naan,60,Breads & Rice,,veg,false,false,,,,10,true\n";
...
export function parseImportRows(text, format, { pureVeg = false } = {}) {
  ...
  rows.forEach((row, idx) => {
    ...
    items.push({
      ...
      etaMinutes: parseInt(get("etaminutes", "etaMinutes", "eta", "ETA", "prep_time", "prep time")) || 15,
      bogoEnabled: truthy(get("bogo", "BOGO", "bogoenabled", "bogoEnabled", "buy1get1")),
      // "Small:180|Medium:250|Large:320" in one cell, not three separate rows —
      // see parseVariationsAddons. Three rows would import as three unrelated
      // menu items instead of one dish a diner picks a size for.
      // Read directly off the row (not through get(), which stringifies) so a
      // JSON row's real array of {name, price} objects survives — this row's
      // keys keep their original case, unlike normalizeMenuRows's rows.
      variations: parseVariationsAddons(row.variations ?? row.Variations ?? row.sizes ?? row.Sizes, "var"),
      addons: parseVariationsAddons(row.addons ?? row.Addons ?? row["add-ons"] ?? row["add_ons"], "add"),
      isCombo: false,
      available: true,
    });
  });
...
// ---------------------------------------------------------------------------
// ZIP (CSV + photos) import
// ---------------------------------------------------------------------------
//
// Lifted out of the outlet importer so the brand master-menu importer can get
// the same "upload a zip of dishes and photos together" capability rather
// than being limited to CSV/JSON paste, which has no way to attach a local
// photo file at all. Written once here instead of twice, the same reasoning
// that already applies to the CSV parsing above.

/**
 * Unzips an uploaded file, locates the first .csv inside it, and collects
 * every image file into a flat { filename: JSZipObject } map regardless of
 * which subfolder it's actually in, so "images/x.jpg" is keyed just as
 * "x.jpg" — matchImageFile above looks it up by filename alone.
 */
...
/**
 * Runs async tasks with a limited number in flight at once (default 5)
 * instead of firing everything simultaneously or waiting for fixed batches to
 * finish — keeps the browser and Cloudinary from being hit all at once, while
 * still uploading continuously with no idle gaps between "rounds".
 */
export async function uploadWithConcurrency(tasks, concurrency, onProgress) {
  let nextIndex = 0;
  let doneCount = 0;

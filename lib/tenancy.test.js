...
describe("homeRouteFor", () => {
  it("sends anyone who manages across outlets to the brand console", () => {
    expect(homeRouteFor(ROLES.BRAND_OWNER)).toBe("/brand");
    expect(homeRouteFor(ROLES.BRAND_MANAGER)).toBe("/brand");
    expect(homeRouteFor(ROLES.OUTLET_MANAGER)).toBe("/brand");
  });

  it("sends floor staff to the surface they actually work", () => {
    expect(homeRouteFor(ROLES.RECEPTION)).toBe("/receptionist");
    expect(homeRouteFor(ROLES.KITCHEN)).toBe("/kitchen");
  });

  it("returns nothing for someone with no role, so login does not bounce them", () => {
    expect(homeRouteFor(null)).toBeNull();
    expect(homeRouteFor("nonsense")).toBeNull();
    expect(homeRouteFor(ROLES.PLATFORM_ADMIN)).toBeNull();
  });
});

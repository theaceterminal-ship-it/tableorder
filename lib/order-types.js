...
export function nextDeliveryAction(order) {
  ...
  // "billed" as well as "ready": handing the order to a rider also generates
  // its bill, so an interrupted dispatch leaves a billed-but-undispatched
  // order. Accepting both means that order can still be sent out instead of
  // losing its button and stranding the food.
  return ["ready", "billed"].includes(order.status) ? "dispatch" : null;
}
...
/**
 * Handed to a rider, not yet delivered.
 *
 * Dispatch also generates the bill, so such an order has status "billed" while
 * still being very much in progress. Reception's board keys off status, so
 * without this it would drop out of "In Kitchen" the instant it was dispatched
 * and "Mark delivered" would become unreachable.
 */
export function isInFlightDelivery(order) {
  return deliveryStage(order) === DELIVERY_STAGES.DISPATCHED;
}
...
/**
 * Why a delivery order was refused, in words a hungry customer can act on.
 *
 * Every one of these is a permission-denied from the same rule, so the raw
 * error says only "Missing or insufficient permissions" — true, useless, and
 * indistinguishable between "we are closed", "your code expired" and "the
 * restaurant has not deployed its rules". The causes are listed here so the
 * page can at least say what to do next instead of showing a stack trace.
 */
export function deliveryOrderErrorMessage(err) {
  if (err?.code === "permission-denied") {
    return "We could not place that order. The restaurant may have stopped taking delivery orders, "
      + "or your phone verification may have expired. Please refresh and try again.";
  }

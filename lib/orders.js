...
/**
 * Collapse the duplicate rows a merged bill produces, for any list that shows
 * orders to a human.
 *
 * generateBill deliberately writes the SAME consolidated bill onto every order
 * in a merged party, so each table's own device can display it. That is right
 * for the diner and wrong for every list on the staff side: a party across two
 * tables appears twice, with the full total on both rows, and anyone adding the
 * column up gets double.
 *
 * Unbilled orders pass through untouched -- before a bill exists, two tables'
 * orders really are two separate things the kitchen has to cook.
 */
export function collapseBillSiblings(orders) {
  const out = [];
  const seen = new Set();

  for (const o of orders || []) {
    if (!isRevenueOrder(o)) { out.push(o); continue; }

    // billId is written by generateBill and is the reliable key. Older bills
    // predate it, so fall back to the signature the bill can be recognised by.
    const key = o.billId
      || (Array.isArray(o.mergedTables) && o.mergedTables.length > 1
        ? ["legacy", [...o.mergedTables].sort().join("-"), o.billTotal, o.billSubtotal, startOfDay(o.createdAt)].join("|")
        : `single:${o.id}`);

...
/**
 * What each outlet is actually charging right now, for the master menu view.
 *
 * The master item is a template — outlets own their own price after seeding
 * and can move it freely — so this is read-only, purely for the owner to see
 * at a glance where a live price has drifted from the template, not to
 * enforce or correct anything. Matched by name (lower-cased, trimmed), the
 * same key seedOutletFromMaster and importMasterItems already dedupe on.
 *
 * Keyed two levels deep — outletId, then the item name — so a lookup is
 * `pricing[outletId]?.[name.trim().toLowerCase()]`, undefined when that
 * outlet has never seeded (or has since removed) the item.
 */
export async function fetchOutletMenuPricing(outletIds = []) {
  const pricing = {};
  await Promise.all((outletIds || []).map(async (id) => {
    const byName = {};
    try {
      const snap = await getDocs(collection(db, "restaurants", id, "menuItems"));
      snap.forEach((d) => {
        const data = d.data();
        const key = (data.name || "").trim().toLowerCase();
        if (key) byName[key] = { price: data.price, available: data.available !== false };
      });
    } catch {
      // Left empty — a read failure here shows as "not seeded" rather than
      // blanking the whole master menu view over one outlet's hiccup.
    }
    pricing[id] = byName;
  }));
  return pricing;
  ...
  for (let i = 0; i < toAdd.length; i += 400) {
    ...
    toAdd.slice(i, i + 400).forEach((m) => {
      batch.set(doc(collection(db, "brands", brandId, "menuItems")), {
        ...
        // These four used to be silently dropped here even when the imported
        // file specified them — chefSpecial and featured were parsed out of
        // every CSV/JSON row and then thrown away at the write, and variations/
        // add-ons had no column to come from at all until now.
        chefSpecial: !!m.chefSpecial,
        featured: !!m.featured,
        bogoEnabled: !!m.bogoEnabled,
...
/**
 * Someone editing their own name and contact.
 *
 * Which document holds that depends on how they got their access: owners and
 * managers are brand members, reception and kitchen are rostered on outlets.
 * Writing blindly to brands/{b}/members/{uid} fails for floor staff, because
 * that document does not exist for them.
 *
 * Role and outlet assignment are never included in the write, and the rules
 * pin them to their existing values, so this cannot become self-promotion.
 */
export async function updateMyProfile({ brandId, uid, scope, outletIds = [], name, phone }) {
  const patch = { name: (name || "").trim(), phone: (phone || "").trim() };

  if (scope === "brand" && brandId) {
    await updateDoc(doc(db, "brands", brandId, "members", uid), patch);
    return;
  }

  // Floor staff can be rostered at more than one outlet; keep every copy of
  // their record consistent rather than leaving stale names on other outlets.
  const targets = outletIds.filter(Boolean);
  if (targets.length === 0) throw new Error("No profile record to update.");
  await Promise.all(targets.map((outletId) =>
    updateDoc(doc(db, "restaurants", outletId, "staff", uid), patch)
  ));
}
...
export async function addMasterItem(brandId, item) {
  ...
  return addDoc(collection(db, "brands", brandId, "menuItems"), {
    ...
    // No form field for these yet on the master-menu "Add item" card, but
    // written through here (rather than hardcoded false/[]) so a caller that
    // does supply them — importMasterItems, or a future editor — isn't
  ...
  for (let i = 0; i < toAdd.length; i += 400) {
    ...
    toAdd.slice(i, i + 400).forEach((m) => {
      ...
      batch.set(ref, {
        ...
        etaMinutes: m.etaMinutes || 15,
        available: true,
        // Same bug as importMasterItems/addMasterItem had: these were
        // hardcoded to empty/false here regardless of what the master item
        // actually carried, so a master dish with sizes or a chef's-special
        // flag seeded onto a new outlet as a plain, flagless item every time.
        featured: !!m.featured,
        chefSpecial: !!m.chefSpecial,
        isCombo: false,
        bogoEnabled: !!m.bogoEnabled,

...
import {
  ...
  deliveryTimeline, isDeliveryComplete, isInFlightDelivery, deliveryOrderErrorMessage,
} from "./order-types";
...
describe("explaining a refused delivery order", () => {
  it("turns a permission denial into something actionable", () => {
    // The raw code is the same whether the restaurant closed, the code
    // expired, or the rules were never deployed — so the message covers the
    // ones a customer can actually do something about.
    const msg = deliveryOrderErrorMessage({ code: "permission-denied" });
    expect(msg).toContain("refresh");
    expect(msg).not.toContain("permission");
  });

  it("distinguishes a network problem", () => {
    expect(deliveryOrderErrorMessage({ code: "unavailable" })).toContain("connection");
  });

  it("still says something useful for an unknown failure", () => {
    for (const e of [{}, null, undefined, new Error("boom")]) {
      expect(deliveryOrderErrorMessage(e)).toBeTruthy();
    }
  });
});

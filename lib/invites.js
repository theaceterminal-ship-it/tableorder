...
export async function listInvites(brandId) {
  ...
  // Consumed invitations stay in the collection as a record of who accepted
  // what, but they are not outstanding, so they do not belong in this list.
  return snap.docs
...
export async function acceptInvite(invite, user) {
  ...
  // Mark the invitation consumed. Kept rather than deleted so there is a record
  // of who accepted what and when, but no longer listed as outstanding.
  //
  // Deliberately last, and deliberately not fatal: the membership above is what
  // actually grants access. If this write fails the person is still correctly
  // signed in, and the only symptom is a stale row in the invitations list --
  // far better than failing an accept that already succeeded.
  try {
    await setDoc(doc(db, "staffInvites", inviteKey(invite.email || user.email)), {
      active: false,
      acceptedAt: Date.now(),
      acceptedByUid: user.uid,
    }, { merge: true });
  } catch (e) {
    console.warn("Invitation accepted, but could not be marked consumed:", e?.code || e.message);
  }

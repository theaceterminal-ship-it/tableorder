...
/**
 * The precomputed recommendation model — a small per-item partner list, not
 * raw orders. Public read by rule (recModels/current), written only by the
 * offline batch job (scripts/build-rec-models.mjs) under the Admin SDK, which
 * bypasses these rules entirely. `null` while it does not exist yet, which a
 * brand-new outlet handles fine: lib/recommendations.js falls back to a
 * category prior with no model at all.
 */
export function useRecModel(outletId) {
  return useOutletDoc(outletId, "recModels", "current").data;
}
...
/**
 * What a diner told us when they requested the bill, keyed by the
 * bill_requested order's own id — a payment method and an optional name and
 * phone. Staff-only by rule; the diner who submitted it can never read it
 * back, since nothing on their side ever needs to.
 */
export function useBillRequestDetails(outletId) {
  const constraints = useMemo(
    () => [where("createdAt", ">=", receptionOrderWindowStart())],
    // eslint-disable-next-line react-hooks/exhaustive-deps
    []
  );
  const { docs } = useOutletCollection(outletId, "billRequestDetails", constraints);
  return useMemo(() => Object.fromEntries(docs.map((d) => [d.id, d])), [docs]);
}

/**
 * Who is carrying each delivery, keyed by order id.
 *
 * Kept off the order document for the same reason deliveryDetails is: orders
 * are broadly readable so a customer can track their own, and a rider's name
 * and phone briefly living there meant anyone who LISTED the collection —
 * not just someone tracking one known order — could enumerate every rider an
 * outlet has ever used. Listing this collection is staff-only by rule;
 * fetching one assignment by its own order id (how the tracker screen reads
 * it) stays open, same trust level the order id itself already carries.
 */
export function useRiderAssignments(outletId) {
  const constraints = useMemo(
    () => [where("createdAt", ">=", receptionOrderWindowStart())],
    // eslint-disable-next-line react-hooks/exhaustive-deps
    []
  );
  const { docs } = useOutletCollection(outletId, "riderAssignments", constraints);
  return useMemo(() => Object.fromEntries(docs.map((d) => [d.id, d])), [docs]);
}
